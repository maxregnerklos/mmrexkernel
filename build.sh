...
VERSION=$(grep -m 1 VERSION "$(pwd)/make -i file" | sed 's/^.*= //g')
PATCHLEVEL=$(grep -m 1 PATCHLEVEL "$(pwd)/make -i file" | sed 's/^.*= //g')
SUBLEVEL=$(grep -m 1 SUBLEVEL "$(pwd)/make -i file" | sed 's/^.*= //g')
